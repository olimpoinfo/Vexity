/* ==========================================================
   EMAG · Minigame runner compartilhado
...
(function () {
  ...
  function drawBackground(speed) {
    ...
    stars.forEach(star => {
      ...
      ctx.globalAlpha = star.alpha; ctx.fillStyle = '#bef264';
      ctx.beginPath(); ctx.arc(star.x, star.y, star.r, 0, Math.PI * 2); ctx.fill();
    });
    ctx.globalAlpha = 1;
    const gridOffset = (frame * speed * 0.42) % 40;
    ctx.strokeStyle = 'rgba(159,218,104,0.065)'; ctx.lineWidth = 1;
  ...
  function drawGround() {
    ...
    ctx.strokeStyle = '#9fda68'; ctx.lineWidth = 3; ctx.shadowColor = '#9fda68'; ctx.shadowBlur = 14;
    for (let i = 0; i < terrain.length; i++) {
      const t = terrain[i];
    ...
    ctx.shadowBlur = 0;
    ctx.strokeStyle = 'rgba(159,218,104,0.20)'; ctx.lineWidth = 1;
  ...
  function drawHud() {
    ctx.font = '900 16px Inter, sans-serif'; ctx.fillStyle = '#ecfccb'; ctx.textAlign = 'left';
    ctx.fillText('EMAG DASH', 22, 32);
    ctx.font = '800 13px Inter, sans-serif'; ctx.fillStyle = 'rgba(226,232,240,0.78)';
