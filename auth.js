/* ============================================================
   Vexity Controle Contábil — Autenticação (armazenamento local)
   ------------------------------------------------------------
...
const AUTH_DEFAULT_SITE_NAME = 'Vexity Controle Contábil';
const AUTH_DEFAULT_BRAND_MAIN = 'VEXITY';
const AUTH_DEFAULT_BRAND_SUB = 'CONTROLE CONTÁBIL';
let AUTH_VEXITY_MARK_SEQ = 0;

function authVexityMarkSvg() {
  const id = `auth-vx-${++AUTH_VEXITY_MARK_SEQ}`;
  return `
    <svg class="vx-mark" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
      <defs>
        <linearGradient id="${id}-grad" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stop-color="var(--vx-grad-1, #c5f28a)"/>
          <stop offset="45%" stop-color="var(--vx-grad-2, #9fda68)"/>
          <stop offset="100%" stop-color="var(--vx-grad-3, #5aa73e)"/>
        </linearGradient>
        <linearGradient id="${id}-fill" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stop-color="var(--vx-grad-2, #9fda68)" stop-opacity="0.9"/>
          <stop offset="100%" stop-color="var(--vx-grad-3, #3d7a24)" stop-opacity="0.9"/>
        </linearGradient>
        <filter id="${id}-glow" x="-24%" y="-24%" width="148%" height="148%">
          <feGaussianBlur stdDeviation="1.2" result="blur"/>
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
      </defs>
      <path class="vx-shield" d="M32 4 L58 18 L58 46 L32 60 L6 46 L6 18 Z"
            fill="url(#${id}-fill)"
            stroke="url(#${id}-grad)"
            stroke-width="1.2"
            stroke-opacity="var(--vx-shield-stroke-opacity, 0.55)"/>
      <path class="vx-letter" d="M18 20 L32 46 L46 20"
            fill="none"
            stroke="url(#${id}-grad)"
            stroke-width="5.5"
...
// Restringe uma lista de empresas ao cliente (escritório) da sessão atual.
// Cada empresa deve carregar um campo `clientId` vinculando-a ao cliente dono
// da carteira. Sem essa restrição, todos os clientes compartilhavam a mesma
// lista de empresas em um único localStorage global — a causa do vazamento.
function authScopeCompaniesToClient(companies, session) {
  const list = Array.isArray(companies) ? companies : [];
  const client = authGetClientForSession(session);
  if (client) return list.filter(c => String(c.clientId || '') === String(client.id));
  // Sessão sem cliente resolvido (ex.: admin master fora do modo visualização):
  // mostra apenas empresas ainda não migradas/vinculadas a nenhum cliente.
  return list.filter(c => !c.clientId);
}
...
function authFilterCompaniesForSession(companies) {
  ...
  const scoped = authScopeCompaniesToClient(list, session);
  if (authIsManager(session)) return scoped;
  if (!authOperatorHasAssignmentRecord(session.email)) return [];
  const assigned = new Set(authGetAssignedCompanyIds(session.email));
  return scoped.filter(c => assigned.has(String(c.id)));
}
...
function authMergeCompanyScopeForSave(storageKey, visibleCompanies) {
  ...
  // Operador ainda sem carteira definida: nunca grava, para não apagar a base
  // ao salvar uma lista vazia.
  if (!authIsManager(session) && !authOperatorHasAssignmentRecord(session.email)) return full;

  const client = authGetClientForSession(session);

  // Garante que toda empresa visível para esta sessão saia salva vinculada ao
  // cliente correto (novas empresas criadas por este cliente, por exemplo).
  const stamped = visible.map(c => (client ? { ...c, clientId: client.id } : { ...c }));

  // Empresas de QUALQUER OUTRO cliente permanecem intocadas.
  const others = full.filter(c => {
    if (client) return String(c.clientId || '') !== String(client.id);
    return !!c.clientId;
  });

  // Dentro do próprio escopo do cliente, atualiza o que foi editado/criado e
  // preserva o que não estava no recorte visível desta sessão (ex.: empresas
  // fora da carteira do operador que salvou).
  const ownScopeFull = client ? full.filter(c => String(c.clientId || '') === String(client.id)) : full.filter(c => !c.clientId);
  const stampedById = new Map(stamped.map(c => [String(c.id), c]));
  const mergedOwn = ownScopeFull.map(c => stampedById.get(String(c.id)) || c);
  stamped.forEach(c => {
    if (!mergedOwn.some(m => String(m.id) === String(c.id))) mergedOwn.push(c);
  });

  return [...others, ...mergedOwn];
}

// --------- migração: empresas antigas sem cliente vinculado ---------
// Antes desta correção, empresas eram salvas sem `clientId`. Estas funções
// permitem ao Administrador Master vincular manualmente, de forma explícita
// e auditável, empresas legadas a um cliente específico — em vez de uma
// atribuição automática, que poderia repetir o mesmo tipo de vazamento.
function authGetUnassignedCompanies(storageKey) {
  if (!authIsAdmin()) return [];
  let full = [];
  try { full = JSON.parse(localStorage.getItem(storageKey)) || []; } catch (e) { full = []; }
  if (!Array.isArray(full)) full = [];
  return full.filter(c => !c.clientId);
}

function authClaimUnassignedCompanies(storageKey, clientId) {
  if (!authIsAdmin()) return { ok: false, msg: 'Apenas o Administrador Master pode migrar empresas.' };
  const client = authFindClient(clientId);
  if (!client) return { ok: false, msg: 'Cliente inválido.' };
  let full = [];
  try { full = JSON.parse(localStorage.getItem(storageKey)) || []; } catch (e) { full = []; }
  if (!Array.isArray(full)) full = [];
  let count = 0;
  const next = full.map(c => {
    if (!c.clientId) { count += 1; return { ...c, clientId: client.id }; }
    return c;
  });
...
// Operadores visíveis para a sessão atual, restritos ao cliente do gerente
// logado (ou em modo de visualização). authGetOperatorUsers() continua
// retornando a lista global e deve ser usada só em telas de auditoria do
// Administrador Master.
function authGetOperatorUsersForSession() {
  const operators = authGetOperatorUsers();
  const session = authGetSession();
  if (!session) return operators;
  const client = authGetClientForSession(session);
  if (authIsAdmin(session) && !client) return operators;
  const clientId = client ? client.id : '';
  return operators.filter(u => String(u.clientId || '') === String(clientId));
}
...
function authClampColorChannel(value) {
  return Math.max(0, Math.min(255, Math.round(Number(value) || 0)));
}

function authRgbToHex(rgb) {
  return `#${[rgb.r, rgb.g, rgb.b].map(v => authClampColorChannel(v).toString(16).padStart(2, '0')).join('')}`;
}

function authMixRgb(rgb, target, amount) {
  const pct = Math.max(0, Math.min(1, Number(amount) || 0));
  return {
    r: rgb.r + (target.r - rgb.r) * pct,
    g: rgb.g + (target.g - rgb.g) * pct,
    b: rgb.b + (target.b - rgb.b) * pct
  };
}

function authApplyVexityLogoPalette(color, rgb) {
  const root = document.documentElement;
  const base = rgb || authHexToRgb(color) || { r: 159, g: 218, b: 104 };
  const light = authRgbToHex(authMixRgb(base, { r: 255, g: 255, b: 255 }, 0.42));
  const dark = authRgbToHex(authMixRgb(base, { r: 0, g: 0, b: 0 }, 0.34));
  root.style.setProperty('--vx-grad-1', light);
  root.style.setProperty('--vx-grad-2', color);
  root.style.setProperty('--vx-grad-3', dark);
  root.style.setProperty('--vx-sub-color', `rgba(${base.r}, ${base.g}, ${base.b}, .70)`);
  root.style.setProperty('--vx-mark-shadow', `rgba(${base.r}, ${base.g}, ${base.b}, .38)`);
  root.style.setProperty('--vx-shield-opacity', '.18');
  root.style.setProperty('--vx-shield-stroke-opacity', '.72');
}


function authEnsureVexityBrandStyles() {
  if (document.getElementById('vexity-brand-styles')) return;
  const style = document.createElement('style');
  style.id = 'vexity-brand-styles';
  style.textContent = `
    :root {
      --vx-grad-1: #c5f28a;
      --vx-grad-2: var(--client-brand-color, #9fda68);
      --vx-grad-3: #5aa73e;
      --vx-shield-opacity: .14;
      --vx-shield-stroke-opacity: .55;
      --vx-sub-color: rgba(var(--client-brand-rgb, 159, 218, 104), .70);
      --vx-mark-shadow: rgba(var(--client-brand-rgb, 159, 218, 104), .35);
    }
    html.emag-theme-light {
      --vx-grad-1: #7ac04a;
      --vx-grad-2: #5aa73e;
      --vx-grad-3: #2f6b1d;
      --vx-shield-opacity: .10;
      --vx-shield-stroke-opacity: .70;
      --vx-sub-color: rgba(74, 141, 50, .72);
      --vx-mark-shadow: rgba(106, 176, 76, .28);
    }
    .emag-top-title, .brand, .auth-about-emag {
    ...
    .vexity-logo .vx-mark {
      ...
      height: 100%;
      display: block;
      filter: drop-shadow(0 4px 12px var(--vx-mark-shadow));
    }
    .vexity-logo .vx-shield {
      opacity: var(--vx-shield-opacity);
    }
    .emag-top-title:not(.client-brand) .emag,
    .brand:not(.client-brand) .emag,
    .auth-about-emag .vexity-brand-text > span {
      background: linear-gradient(135deg, var(--vx-grad-1) 0%, var(--vx-grad-2) 45%, var(--vx-grad-3) 100%) !important;
    ...
    .emag-top-title .emag, .brand .emag {
      letter-spacing: 3px !important;
      line-height: 1 !important;
      font-weight: 800 !important;
    }
    .emag-top-title .contabilidade, .brand .sub, .auth-about-emag .vexity-brand-text > small {
      color: var(--vx-sub-color) !important;
      letter-spacing: 3px !important;
    ...
    .emag-top-title.client-brand .emag,
    .brand.client-brand .emag,
    .emag-top-title.client-brand .contabilidade,
    .brand.client-brand .sub {
      background: none !important;
      -webkit-text-fill-color: currentColor !important;
    }
    .emag-top-title.brand-extra-long .vexity-logo,
    .brand.brand-extra-long .vexity-logo {
      width: 34px;
      height: 34px;
      flex-basis: 34px;
    }
    @media (max-width: 620px) {
      .vexity-logo { width: 32px; height: 32px; flex-basis: 32px; }
      .emag-top-title, .brand, .auth-about-emag { gap: 8px !important; }
      .emag-top-title .emag, .brand .emag { letter-spacing: 2px !important; }
      .emag-top-title .contabilidade, .brand .sub { letter-spacing: 1.4px !important; }
    }
  `;
  document.head.appendChild(style);
}

function authBuildFaviconSvg(color, rgb) {
  const base = rgb || authHexToRgb(color) || { r: 159, g: 218, b: 104 };
  const mid = color || authRgbToHex(base);
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">` +
    `<path d="M32 4 L58 18 L58 46 L32 60 L6 46 L6 18 Z" fill="${mid}"/>` +
    `<path d="M18 20 L32 46 L46 20" fill="none" stroke="#000000" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/>` +
    `</svg>`;
}

function authApplyFavicon(color, rgb) {
  try {
    const svg = authBuildFaviconSvg(color, rgb);
    const dataUrl = 'data:image/svg+xml,' + encodeURIComponent(svg);
    let link = document.getElementById('vexity-favicon');
    if (!link) {
      link = document.createElement('link');
      link.id = 'vexity-favicon';
      link.rel = 'icon';
      link.type = 'image/svg+xml';
      document.head.appendChild(link);
    }
    link.href = dataUrl;
    document.querySelectorAll('link[rel="icon"], link[rel="shortcut icon"]').forEach(l => { if (l !== link) l.remove(); });
  } catch (e) { /* silencioso: favicon é cosmético */ }
}

function authEnsureVexityBranding() {
  if (!document.body) return;
  authApplyFavicon('#9fda68');
  authEnsureVexityBrandStyles();
  document.querySelectorAll('.emag-top-title, .brand').forEach(logo => {
    let mark = logo.querySelector(':scope > .vexity-logo');
    let textWrap = logo.querySelector(':scope > .vexity-brand-text');
    if (!textWrap) {
      textWrap = document.createElement('span');
      textWrap.className = 'vexity-brand-text';
      Array.from(logo.childNodes).forEach(node => {
        if (node.nodeType === 1 && node.classList.contains('vexity-logo')) return;
        textWrap.appendChild(node);
      });
    }
    if (!mark) {
      mark = document.createElement('span');
      mark.className = 'vexity-logo';
      mark.setAttribute('aria-hidden', 'true');
    }
    mark.className = 'vexity-logo';
    mark.setAttribute('aria-hidden', 'true');
    mark.innerHTML = authVexityMarkSvg();
    if (mark.parentNode !== logo || textWrap.parentNode !== logo) {
      logo.appendChild(mark);
      logo.appendChild(textWrap);
    }
    logo.setAttribute('title', AUTH_DEFAULT_SITE_NAME);
    logo.setAttribute('aria-label', AUTH_DEFAULT_SITE_NAME);
    const main = logo.querySelector('.emag');
    const sub = logo.querySelector('.contabilidade, .sub');
    if (main && (!main.textContent.trim() || main.textContent.trim().toUpperCase() === 'EMAG')) main.textContent = AUTH_DEFAULT_BRAND_MAIN;
    if (sub && (!sub.textContent.trim() || sub.textContent.trim().toUpperCase() === 'CONTABILIDADE')) sub.textContent = AUTH_DEFAULT_BRAND_SUB;
  });
}
...
function authApplyClientBranding() {
  ...
  authApplyVexityLogoPalette(color, rgb);
  authApplyFavicon(color, rgb);
  ...
  // Aplicar cor dinâmica aos elementos de perfil, setas e capas
  authApplyDynamicBrandingStyles(color, rgb);

  if (document.title && document.title.includes('Vexity Controle Contábil')) {
    document.title = document.title.replace('Vexity Controle Contábil', siteName);
  }
}

/* Aplica estilos dinâmicos para anel de perfil, setas, capas e outros elementos */
function authApplyDynamicBrandingStyles(color, rgb) {
  if (!document.getElementById('auth-dynamic-branding-styles')) {
    const style = document.createElement('style');
    style.id = 'auth-dynamic-branding-styles';
    style.textContent = `
      /* Anel de perfil do usuário */
      #auth-badge-btn .auth-avatar {
        border: 2px solid var(--client-brand-color, #9fda68) !important;
        box-shadow: 0 0 0 2px var(--bg, #0f1117), 0 0 12px rgba(var(--client-brand-rgb, 159, 218, 104), 0.35) !important;
      }
      
      /* Setas de navegação */
      .module-arrow, .cp-arrow, .auth-dd-ico {
        color: var(--client-brand-color, #9fda68) !important;
      }
      
      /* Capas/placeholders de perfil */
      .auth-photo-preview {
        background: linear-gradient(135deg, var(--client-brand-color, #9fda68), rgba(var(--client-brand-rgb, 159, 218, 104), 0.6)) !important;
        border: 2px solid var(--client-brand-color, #9fda68) !important;
        box-shadow: 0 0 0 1px var(--bg, #0f1117), 0 0 12px rgba(var(--client-brand-rgb, 159, 218, 104), 0.25) !important;
      }
      
      /* Avatar em modais e dropdowns */
      #auth-dropdown .auth-avatar {
        border: 2px solid var(--client-brand-color, #9fda68) !important;
        box-shadow: 0 0 0 2px var(--surface, #1a1d27), 0 0 8px rgba(var(--client-brand-rgb, 159, 218, 104), 0.3) !important;
      }
      
      /* Pontos de status e indicadores */
      .cp-dot {
        background: var(--client-brand-color, #9fda68) !important;
      }
      
      /* Badges e pills de status */
      .role-pill {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.35) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.08) !important;
        color: var(--client-brand-color, #9fda68) !important;
      }
      
      /* Crop stage (foto) */
      .auth-crop-stage {
        box-shadow: 0 0 0 4px rgba(var(--client-brand-rgb, 159, 218, 104), 0.12) !important;
      }
      
      /* Slider de zoom */
      .auth-range {
        background: linear-gradient(90deg, var(--client-brand-color, #9fda68), rgba(var(--client-brand-rgb, 159, 218, 104), 0.5)) !important;
      }
      
      .auth-range::-webkit-slider-thumb {
        border-color: var(--client-brand-color, #9fda68) !important;
      }
      ...
      /* Modo claro: ajustes adicionais */
      html.emag-theme-light #auth-badge-btn .auth-avatar {
        box-shadow: 0 0 0 2px #ffffff, 0 0 0 4px var(--client-brand-color, #9fda68) !important;
      }
      
      html.emag-theme-light .auth-photo-preview {
        box-shadow: 0 0 0 1px #ffffff, 0 0 8px rgba(var(--client-brand-rgb, 159, 218, 104), 0.2) !important;
      }
      ...
      /* Pills e badges */
      .pill {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.22) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.08) !important;
        color: var(--client-brand-color, #9fda68) !important;
      }
      ...
      /* Botão secundário com foco */
      input:focus, select:focus, textarea:focus {
        border-color: var(--client-brand-color, #9fda68) !important;
        box-shadow: 0 0 0 3px rgba(var(--client-brand-rgb, 159, 218, 104), 0.08) !important;
      }
      
      /* Checkbox e radio buttons */
      input[type="checkbox"], input[type="radio"] {
        accent-color: var(--client-brand-color, #9fda68) !important;
      }
      
      /* Status ativo */
      .status {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.28) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.10) !important;
        color: var(--client-brand-color, #9fda68) !important;
      }
      
      /* Linha de empresa selecionada */
      .company-row.selected {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.38) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.07) !important;
      }
      
      .company-row:hover {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.24) !important;
      }
      
      /* Resumo do operador */
      .operator-summary {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.18) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.055) !important;
      }
      ...
      /* Modo claro: ajustes adicionais para novos elementos */
      html.emag-theme-light .pill {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.3) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.08) !important;
      }
      
      html.emag-theme-light .operator-summary {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.25) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.06) !important;
      }
      
      html.emag-theme-light .company-row.selected {
        border-color: rgba(var(--client-brand-rgb, 159, 218, 104), 0.4) !important;
        background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.08) !important;
      }
      
      /* Título de seção em configurações */
      .auth-setting-heading {
        color: var(--client-brand-color, #9fda68) !important;
      }
      
      /* Placeholder de avatar (iniciais) */
      .auth-avatar:not(:has(img)) {
        background: linear-gradient(135deg, var(--client-brand-color, #9fda68), rgba(var(--client-brand-rgb, 159, 218, 104), 0.6)) !important;
        color: #0f1117 !important;
      }
      
      /* Placeholder de foto de perfil em modais */
      .auth-photo-preview:not(:has(img)) {
        background: linear-gradient(135deg, var(--client-brand-color, #9fda68), rgba(var(--client-brand-rgb, 159, 218, 104), 0.6)) !important;
        color: #0f1117 !important;
      }
    `;
...
function authUpsertClient(data) {
  ...
  if (!id) return { ok: false, msg: 'Informe o ID numérico do cliente.' };
  if (!/^\d+$/.test(id)) return { ok: false, msg: 'O ID do cliente deve conter apenas números.' };
  const clients = authGetClients();
  const editingId = String(d.originalId || d.editingId || '').trim();
  const duplicate = clients.find(c => c.id !== editingId && c.id === id);
  if (duplicate) return { ok: false, msg: 'Já existe um cliente com este ID.' };
  const previous = clients.find(c => c.id === editingId || c.id === id) || {};
  const record = {
    id,
    nome,
    siteTitle: String(d.siteTitle || nome).trim(),
    slug: String(d.slug || authSlugify(nome) || id).trim(),
  ...
  if (editingId && editingId !== id) {
    ...
    let changed = false;
    users.forEach(u => {
      if (u.clientId === editingId) { u.clientId = id; changed = true; }
    });
    if (changed) authSaveUsers(users);
    if (authGetPreviewClient && authGetPreviewClient() === editingId) authSetPreviewClient(id);
  }
...
function authCreateManager(nome, email, senha, meta) {
  ...
  if (res.ok && meta) {
    // O vínculo com o cliente só pode ser definido AGORA, na criação. Depois
    // disso o gerente é intransferível (ver authUpdateManager).
    ...
    if (clientId) {
      ...
      if (idx !== -1) {
        users[idx].clientId = clientId;
        authSaveUsers(users);
      }
    }
    const { clientId: _omitClientId, ...rest } = meta;
    authUpdateManager(email, rest);
  }
  return res;
}
...
function authUpdateManager(email, fields) {
  ...
  // Gerentes são intransferíveis: o cliente vinculado só é definido na
  // criação (authCreateManager) e nunca pode ser alterado depois. Qualquer
  // tentativa de mudar o clientId de um gerente já existente é bloqueada.
  if (Object.prototype.hasOwnProperty.call(f, 'clientId')) {
    const requested = String(f.clientId || '').trim();
    const current = String(users[idx].clientId || '').trim();
    if (requested !== current) {
      return { ok: false, msg: 'Gerentes são intransferíveis entre clientes. Para mudar o cliente, exclua este gerente e cadastre um novo vinculado ao cliente correto.' };
    }
  }
  if (Object.prototype.hasOwnProperty.call(f, 'cargo')) users[idx].cargo = String(f.cargo || '').trim();
  if (Object.prototype.hasOwnProperty.call(f, 'telefone')) users[idx].telefone = String(f.telefone || '').trim();
...
/* Insere um chip com o nome/avatar do usuário no cabeçalho de cada
   página protegida, posicionado à ESQUERDA da logo Vexity. Ao clicar,
...
function authMountBadge(opts) {
  ...
  if (header) {
    ...
    // incluindo a logo Vexity, que é sempre o primeiro elemento do header.
    ...
    <div class="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-modal-about-title">
      ...
      <div class="auth-modal-body">
        <div class="auth-about-brand">
          <div class="auth-about-emag"><span class="vexity-logo" aria-hidden="true">${authVexityMarkSvg()}</span><span class="vexity-brand-text"><span>VEXITY</span><small>CONTROLE CONTÁBIL</small></span></div>
        </div>
...
function authInjectStyles() {
  ...
  style.textContent = `
    ...
    #auth-badge-btn:focus-visible { outline: 2px solid rgba(var(--client-brand-rgb, 159, 218, 104),0.5); outline-offset: 3px; }
    #auth-badge-btn[aria-expanded="true"] .auth-avatar {
      box-shadow: 0 0 0 2px #0f1117, 0 0 0 4px var(--client-brand-color, #9fda68), 0 0 12px rgba(var(--client-brand-rgb, 159, 218, 104),0.55);
    }
    ...
    #auth-badge-btn.has-name {
      ...
      border: 1px solid rgba(var(--client-brand-rgb, 159, 218, 104),0.22);
      border-radius: 100px;
      padding: 4px 14px 4px 4px;
    }
    #auth-badge-btn.has-name:hover { background: rgba(var(--client-brand-rgb, 159, 218, 104),0.08); border-color: rgba(var(--client-brand-rgb, 159, 218, 104),0.4); transform: none; }
    #auth-badge-btn.has-name:active { transform: none; }

    .auth-avatar {
      width: 34px; height: 34px; border-radius: 50%;
      background: linear-gradient(135deg, var(--client-brand-color, #9fda68), #4f6ef7);
      color: #0f1117; font-weight: 800; font-size: 12.5px;
      display: flex; align-items: center; justify-content: center;
      flex-shrink: 0; overflow: hidden;
      box-shadow: 0 0 0 2px #0f1117, 0 0 0 4px var(--client-brand-color, #9fda68);
    ...
    .auth-dd-role {
      ...
      border: 1px solid rgba(var(--client-brand-rgb, 159, 218, 104),0.16);
      border-radius: 10px;
      background: linear-gradient(135deg, rgba(var(--client-brand-rgb, 159, 218, 104),0.08), rgba(79,110,247,0.05));
      margin-bottom: 8px;
    }
    .auth-dd-role span { display: block; color: var(--client-brand-color, #9fda68); font-size: 12px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.4px; }
    ...
    .auth-dd-item:hover { background: rgba(var(--client-brand-rgb, 159, 218, 104),0.1); }
    ...
    .auth-banner.success { background: rgba(var(--client-brand-rgb, 159, 218, 104),0.12); color: var(--client-brand-color, #9fda68); border: 1px solid rgba(var(--client-brand-rgb, 159, 218, 104),0.25); }
    ...
    .auth-btn-secondary:hover { background: #2a2f47; border-color: var(--client-brand-color, #9fda68); }
    ...
    .auth-field input:focus { border-color: var(--client-brand-color, #9fda68); background: #22263a; }
    .auth-select:focus { border-color: var(--client-brand-color, #9fda68); background-color: #22263a; }
    ...
    .auth-setting-heading {
      ...
      color: var(--client-brand-color, #9fda68); margin-bottom: 12px;
    }
    .auth-toggle-row {
    ...
    .auth-sound-panel {
      margin-top: 12px;
      padding: 12px;
      border: 1px solid rgba(var(--client-brand-rgb, 159, 218, 104),0.18);
      border-radius: 12px;
      background: linear-gradient(135deg, rgba(var(--client-brand-rgb, 159, 218, 104),0.06), rgba(79,110,247,0.05));
    }
    .auth-range-head {
      display: flex;
      align-items: center;
    ...
    .auth-range-head span {
      color: var(--client-brand-color, #9fda68);
      font-size: 11px;
      font-weight: 800;
    ...
    .auth-range {
      ...
      height: 6px;
      border-radius: 999px;
      background: linear-gradient(90deg, var(--client-brand-color, #9fda68), #4f6ef7);
      outline: none;
      cursor: pointer;
    }
    .auth-range::-webkit-slider-thumb {
      ...
      background: #e2e8f0;
      border: 3px solid var(--client-brand-color, #9fda68);
    ...
    .auth-switch:checked {
      background: rgba(var(--client-brand-rgb, 159, 218, 104),0.28);
      border-color: rgba(var(--client-brand-rgb, 159, 218, 104),0.55);
    }
    .auth-switch:checked::before {
      transform: translateX(20px);
      background: var(--client-brand-color, #9fda68);
    }
    .auth-switch:focus-visible {
      outline: 2px solid rgba(var(--client-brand-rgb, 159, 218, 104),0.45);
      outline-offset: 3px;
    }
    ...
    .auth-btn-primary {
      background: var(--client-brand-color, #9fda68); color: #0f1117; border: none;
    ...
    .auth-about-emag {
      font-size: 22px; font-weight: 800; color: var(--client-brand-color, #9fda68);
      letter-spacing: 3px; text-shadow: 0 0 18px rgba(var(--client-brand-rgb, 159, 218, 104),0.35);
      line-height: 1.1;
    }
    .auth-about-sub {
      font-size: 9px; color: rgba(var(--client-brand-rgb, 159, 218, 104),0.55);
    ...
    .auth-about-version {
      background: rgba(var(--client-brand-rgb, 159, 218, 104),0.12); color: var(--client-brand-color, #9fda68);
    ...
    .auth-crop-stage {
      ...
      border: 1px solid #2e3350;
      box-shadow: 0 0 0 4px rgba(var(--client-brand-rgb, 159, 218, 104),0.12);
      cursor: grab;
    }
    ...
    .auth-crop-controls input[type="range"]::-webkit-slider-thumb {
      ...
      background: var(--client-brand-color, #9fda68); cursor: pointer; border: 2px solid #0f1117;
      box-shadow: 0 0 0 1px rgba(var(--client-brand-rgb, 159, 218, 104),0.4);
    }
    ...
    html.emag-theme-light .auth-avatar {
      box-shadow: 0 0 0 2px #ffffff, 0 0 0 4px var(--client-brand-color, #9fda68);
    }
...
function authEnsureChangeConfirmStyles() {
  ...
  style.textContent = `
    ...
    .auth-change-confirm-card {
      ...
      border: 1px solid rgba(var(--client-brand-rgb, 159, 218, 104), 0.26);
      border-radius: 20px;
    ...
    .auth-change-confirm-icon {
      ...
      display: grid;
      place-items: center;
      background: rgba(var(--client-brand-rgb, 159, 218, 104), 0.14);
      border: 1px solid rgba(var(--client-brand-rgb, 159, 218, 104), 0.36);
      color: var(--client-brand-color, #9fda68);
      font-weight: 900;
      font-size: 18px;
    }
    ...
    .auth-change-confirm-overlay.closing {
      animation: authConfirmFadeOut 140ms ease-in forwards;
    }
    .auth-change-confirm-overlay.closing .auth-change-confirm-card {
      animation: authConfirmPopOut 140ms ease-in forwards;
    }
    @keyframes authConfirmFadeIn { from { opacity: 0; } to { opacity: 1; } }
    @keyframes authConfirmFadeOut { from { opacity: 1; } to { opacity: 0; } }
    @keyframes authConfirmPop { from { opacity: 0; transform: translateY(10px) scale(0.98); } to { opacity: 1; transform: translateY(0) scale(1); } }
    @keyframes authConfirmPopOut { from { opacity: 1; transform: translateY(0) scale(1); } to { opacity: 0; transform: translateY(6px) scale(0.97); } }
  `;
  document.head.appendChild(style);
}
...
function authShowChangeConfirmation(options) {
  ...
  // Fecha sozinho rapidamente (a pessoa também pode fechar antes clicando,
  // apertando Esc/Enter ou clicando fora) — antes esse modal só fechava
  // manualmente e ficava aberto indefinidamente.
  const autoCloseMs = typeof opts.autoCloseMs === 'number' ? opts.autoCloseMs : 2000;
  let autoTimer = autoCloseMs > 0 ? setTimeout(close, autoCloseMs) : null;
}
