  ...
  if (!document.getElementById('emag-celebration-styles')) {
    ...
    s.textContent = `
      ...
      .portfolio-firework {
        ...
        width: 8px; height: 8px; border-radius: 999px; background: var(--client-brand-color, #9fda68);
        box-shadow: 0 0 24px rgba(var(--client-brand-rgb, 159, 218, 104),0.95);
        animation: emagfw-core 900ms ease-out forwards;
      }
      .portfolio-firework span {
        position: absolute; left: 50%; top: 50%;
        width: 5px; height: 5px; border-radius: 999px; background: var(--client-brand-color, #9fda68);
        box-shadow: 0 0 14px rgba(var(--client-brand-rgb, 159, 218, 104),0.95);
      ...
      .portfolio-celebration-toast {
        ...
        border: 1px solid rgba(var(--client-brand-rgb, 159, 218, 104),0.48);
        background: linear-gradient(135deg, rgba(18,29,23,0.96), rgba(21,45,31,0.94));
        color: #ecfccb;
        box-shadow: 0 22px 70px rgba(0,0,0,0.45), 0 0 34px rgba(var(--client-brand-rgb, 159, 218, 104),0.22);
        text-align: center; opacity: 0; pointer-events: none;
        transition: opacity 260ms ease, transform 260ms ease;
      }
      .portfolio-celebration-toast.show { opacity: 1; transform: translate(-50%, 0) scale(1); }
      .portfolio-celebration-toast strong { display: block; font-size: 16px; margin-bottom: 4px; color: var(--client-brand-color, #9fda68); }
